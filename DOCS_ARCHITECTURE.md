...
**Примечание по текущей реализации Phase 1:** глобальный реестр шорткатов живёт в `src/hooks/useKeyboardShortcuts.ts` и вешается на `window` в capture phase. В editable-элементах шорткаты по умолчанию не перехватываются, кроме `Ctrl+S` и `Ctrl+K`.
...
**Что реально есть в конце Phase 1:**
...
- отдельные `CanvasControls` и `MapToolbar`
