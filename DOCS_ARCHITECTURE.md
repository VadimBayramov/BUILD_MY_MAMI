...
16. [Текущая логика и правила (после Phase 3)](#16-текущая-логика-и-правила-после-phase-3)
17. [Рекомендуемый порядок разработки](#17-рекомендуемый-порядок-разработки)
...
**Экраны отображаются как iPhone 14 (220×476px) с полностью интерактивным iframe** — пользователь может кликать по кнопкам, вводить текст в input, выбирать опции прямо на карте. Контент рендерится при ширине менеджера (346px) и масштабируется через `scale(0.618)` — перенос текста идентичен менеджеру.

**Связи между экранами** имеют направляющие стрелки (ArrowClosed) с цветом по статусу: default-path (синий), conditional (жёлтый), error (красный), self-loop (красный), in-cycle (оранжевый).
...
**Текущая реализация (Post-Phase 3):**

- Центральный iPhone 14 (356×770px) с iframe-превью экрана
- iframe заблокирован (`pointer-events: none`) — клики не проходят
- Overlay-кнопки поверх каждого элемента: клик → выделение, drag → перетаскивание (dnd-kit)
- **Двойной клик** по текстовому элементу (heading, paragraph, button, option) → inline textarea для редактирования контента. Enter сохраняет, Escape отменяет
- Навигация: стрелки «<» «>» между экранами, chip-row внизу
- Escape → назад на карту с фокусом на текущем экране
...
| `Ctrl+Y` | Повторить (доп. вариант redo) |
| `Ctrl+C` | Копировать выбранный экран |
| `Ctrl+X` | Вырезать выбранный экран |
| `Ctrl+V` | Вставить |
| `Ctrl+D` | Дублировать |
| `Ctrl+N` | Создать новый экран (quick-add) — позиционируется рядом с выделенным, сразу фокус на имя |
| `Ctrl+A` | Выбрать все экраны на карте |
| `Ctrl+B` | Сгруппировать выделенные экраны в блок |
| `Ctrl+Enter` | Следовать по default-связи от выбранного экрана (навигация по потоку) |
| `Ctrl+Shift+F` | Глобальный поиск экрана по имени или содержимому элементов (модальный QuickSearch) |
| `Ctrl+K` | Открыть/закрыть модалку шорткатов |
| `Escape` | Снять выделение |
| `1` | Режим карты |
| `2` | Режим менеджера |
| `3` | Режим разработчика |

**Примечание по реализации:** глобальный реестр шорткатов живёт в `src/hooks/useKeyboardShortcuts.ts` и вешается на `window` в capture phase. В editable-элементах шорткаты по умолчанию не перехватываются, кроме `Ctrl+S`, `Ctrl+K` и `Ctrl+Shift+F`. Функция `run()` может вернуть `false` — в этом случае `event.preventDefault()` не вызывается и событие обрабатывается браузером нативно (используется для Tab, Space, Home, End при отсутствии контекста).
...
| `Pan / Drag` | Навигация по карте средствами React Flow |
| `Ctrl+Shift+1` | Показать все экраны (fit all) |
| `Ctrl+Shift+L` | Авто-раскладка экранов по цепочке связей (BFS от стартового) |
| `Delete` / `Backspace` | Удалить выбранные экраны и/или выбранные связи |
| `+` / `-` | Zoom in/out |
| `Alt + Drag` | Selection box для группового выделения |
| `F2` | Переименовать экран (фокус на поле Name в правой панели) |
| `F3` | Изменить ID экрана или выделенного элемента (фокус на поле ID/Slug) |
| `Tab` | Перейти к следующему экрану по цепочке (default-связь → любая исходящая). Только при выделенном экране; без выделения — нативный Tab |
| `Shift+Tab` | Перейти к предыдущему экрану по цепочке (входящая связь) |
| `Space` | Переключить превью экрана (toggle `previewVisible`). `panActivationKeyCode` отключён |
| `Home` | Перейти к стартовому экрану (или первому по order) + центрировать камеру |
| `End` | Перейти к конечному экрану (последний в default-цепочке) + центрировать камеру |

**Что реально есть к концу Phase 2:**
- карта построена на `@xyflow/react`
- `MiniMap` кликабельна, pannable и zoomable
- `ConnectionMode.Loose` для удобного соединения экранов
- отдельные `CanvasControls` и `MapToolbar` (кнопки Link, Block, Layout)
- `linkMode` для автосвязи при duplicate/paste
- авто-раскладка (BFS) через кнопку Layout в тулбаре или `Ctrl+Shift+L`
- навигация по цепочке через Tab/Shift+Tab с центрированием камеры на целевой ноде
- QuickSearch (Ctrl+Shift+F) — модальный поиск по имени экрана и содержимому элементов
...
**Хранилище данных:**
- **IndexedDB** (через **Dexie.js** v4.3) — основное хранилище для `projects`, `assets`, `history`. В Phase 1 реально используется сохранение проекта в `projects`; таблицы `assets` и `history` подготовлены как часть схемы.
- **localStorage** — только для мелких UI-настроек: размеры/состояние панелей, лимит undo-истории, последний открытый проект. Данные проекта туда не пишутся.

**Автосохранение (Phase 1):**
- debounce `2s` после изменения `project.updatedAt`
- сохранение идёт в IndexedDB через `storage-service.ts`
- при загрузке приложение открывает `last opened project`, если он есть

**Текущая реализация store:** вместо трёх независимых store проект собран как **один Zustand store на slices** (`funnel`, `ui`, `history`). Это упростило доступ к состоянию, уменьшило связность между модулями и хорошо сочетается с `Immer patches` для undo/redo.
...
CLIPBOARD_DUPLICATE    — дублировать

UI_SET_LINK_MODE       — включить / выключить автосвязку
UI_SELECT_ALL_SCREENS  — выбрать все экраны
UI_SET_GRID_SNAP       — включить / выключить snap-to-grid
UI_SET_SHOW_MINIMAP    — показать / скрыть minimap
HISTORY_SET_MAX        — изменить лимит undo-истории
...
PROJECT_CREATE_NEW     — создать новый проект
PROJECT_EXPORT_HTML    — экспорт всей воронки в HTML/zip:html global-css, js

BLOCK_ADD              — добавить блок-группу на карте
BLOCK_DELETE           — удалить блок
BLOCK_UPDATE           — обновить блок
```
...
function MapCanvas() {
  return (
    <ReactFlow
      nodes={nodes}
      edges={edges}
      nodeTypes={{ screen: ScreenNode, block: BlockNode }}
      edgeTypes={{ screen: ScreenEdge }}
      zoomOnScroll
      zoomOnPinch
      panOnDrag
...
- **Screen Nodes** — кастомные ноды-экраны (`ScreenNode`) с phone-like UI, бейджем типа и порядковым номером
- **Block Nodes** — группирующие блоки (`BlockNode`) с resize через `NodeResizer`
- **Connection Lines** — SVG Bezier-кривые между экранами
- **Selection Box** — групповое выделение через drag при зажатом `Alt`
- **Minimap** — мини-карта в правом нижнем углу
- **Grid** — dots background
- **Top Toolbar** — `MapToolbar` c `Link` и `Block`

### 7.3 Toolbar экрана (при наведении/выделении)

В текущей реализации toolbar показывается **на выбранной ноде**:

- `Duplicate` — дублирует экран через store-action
- `Delete` — удаляет экран
- slot под `Copy` в UI уже есть, но основное копирование сейчас закрывается глобальным `Ctrl+C`

Отдельно над картой есть `MapToolbar`:
- `Link` — включает/выключает автосвязку при duplicate/paste
- `Block` — объединяет выделенные экраны в блок-группу
...
## 15. Файловая структура проекта (актуально на конец Phase 1)

**Текущий стек Phase 1:**
- `Vite 6` + `React 19` + `TypeScript strict`
- `Zustand 5` + `Immer 10` для state и undo/redo
- `@xyflow/react 12` для карты экранов
- `Dexie 4` / `IndexedDB` для persistence
- `Vitest 4` + `happy-dom` для тестов
- `ESLint 9` + `Prettier 3`
...
│
├── src/
│   ├── main.tsx                      ← React entry + dev `window.__store`
│   ├── App.tsx                       ← Root component: AppShell, autosave, загрузка последнего проекта, shortcuts modal
│   ├── vite-env.d.ts
│   │
│   ├── types/
│   │   ├── funnel.ts                 Funnel, Screen, FunnelElement, Connection, Block
│   │   ├── project.ts                ProjectDocument, AssetReference
│   │   ├── ui.ts                     UIState, Mode, HistoryEntry, ClipboardData
│   │   ├── store.ts                  AppState, FunnelStore, actions
│   │   └── index.ts
│   │
│   ├── store/
│   │   ├── funnel-store.ts           Один Zustand store, собранный из slices
│   │   ├── defaults.ts               createDefaultProject / createDefaultScreen
│   │   ├── index.ts
│   │   ├── funnel-store.persistence.test.ts
│   │   ├── middleware/
│   │   │   ├── persist.ts            Dexie schema + UI prefs + last opened project
│   │   │   ├── persist.test.ts
│   │   │   └── undo.ts               вспомогательные заметки / задел под middleware
│   │   └── slices/
│   │       ├── funnel-slice.ts       Проект, экраны, элементы, связи, блоки
│   │       ├── ui-slice.ts           Mode, selection, panels, minimap, linkMode
│   │       └── history-slice.ts      Undo/redo, clipboard, save/load, history limit
│   │
│   ├── components/
│   │   ├── layout/
│   │   │   ├── AppShell.tsx
...
│
│   Часть модулей из полного архитектурного плана пока остаётся future work.
│   Если файла нет в дереве выше, значит он ещё не реализован и не должен считаться source of truth.
│
...
## 16. Текущая логика и правила (после Phase 3)

### 16.1 Два режима отображения экрана

| | Карта (Map Mode) | Менеджер (Manager Mode) |
|---|---|---|
| **Размер телефона** | 220×476px (scale 0.618 от менеджера) | 356×770px |
| **Контент iframe** | Рендерится при 346px, scale(0.618) → текст переносится идентично менеджеру | Рендерится при реальной ширине (~346px) |
| **Интерактивность** | Полная — клики, ввод текста, выбор опций работают прямо на карте | Заблокирована (pointer-events: none на iframe) |
| **Редактирование текста** | Нет inline-редактирования | Двойной клик по overlay → inline textarea |
| **Overlay на элементах** | Нет overlays | Прозрачные overlay-кнопки для выделения и drag |
| **Скроллбар внутри** | Тонкий (3px) вертикальный скроллбар | Scroll через .viewport обёртку |

### 16.2 Телефон на карте — пропорции

```
Менеджер (.phoneFrame)                 Карта (.phone)
356 × 770 px                           220 × 476 px
border: 5px                            border: 3px
border-radius: 54px                    border-radius: 34px
inset (screen): 5px                    inset (screen): 3px
Dynamic Island: 102 × 24px             Dynamic Island: 64 × 15px
border-radius DI: 12px                 border-radius DI: 8px
screen border-radius: 48px             screen border-radius: 30px

Масштаб: 220/356 = 0.618
iframe на карте: width: 346px, scale(0.618) → визуально 214px
```

### 16.3 Режим замка (Lock Mode)

- Включается: `Ctrl+Q` или кнопка замка в `MapToolbar`
- Отключены: zoom scroll/pinch/doubleClick, pan drag, selection drag, node drag, node connect
- Навигация: только WASD / стрелки (шаг 80px, 150ms анимация)
- Handles: рендерятся всегда, в lock mode скрыты через `opacity: 0; pointer-events: none` — связи не пропадают
- iframe: интерактивный всегда (lock mode не влияет)

### 16.4 Связи между экранами

- Визуальные стрелки: `MarkerType.ArrowClosed` с цветом по статусу
- Цветовая карта: default-path `#3b82f6`, conditional `#f59e0b`, error `#f87171`, self-loop `#ef4444`, in-cycle `#f97316`
- `ScreenEdge` деструктурирует `markerEnd` из EdgeProps и передаёт в `<BaseEdge>`
- Ширина линии: 1.5px (2.5px при selected)
- Self-loop: дуга над нодой (archHeight 90px)

### 16.5 Навигация клавишами

| Клавиша | Контекст | Действие |
|---------|----------|----------|
| Space | Менеджер → карта / карта → менеджер | Переключение с зумом 1.5 к выбранному экрану |
| Home | Карта | К стартовому экрану (зум 1.5) |
| End | Карта | К конечному экрану (зум 1.5) |
| Tab | Карта | Следующий по default-связи |
| Shift+Tab | Карта | Предыдущий по default-связи |
| Ctrl+Enter | Карта | Следующий по default-связи |
...
- iframe всегда `pointer-events: auto` — формы, кнопки, input работают
- `sandbox="allow-same-origin"` — необходимо для форм
- Единый `PREVIEW_STYLE`: вертикальный скролл + тонкий скроллбар (3px)
- ReactFlow-drag работает через wrapper `.node`, не через iframe
...
Phase 3 — Парсер и стили (неделя 5-6)
  ...
  ├── Замочек (Lock Mode) + Ctrl+Q
  ├── WASD / Стрелки для навигации в режиме Lock
  ├── Прокрутка контента внутри телефона (Lock mode)
  ├── Space из менеджера → карта (зум ×2 ближе к выбранному экрану)
  ├── Home/End — навигация к start/end экрану с зумом
  ├── Стрелки-маркеры на связях между экранами (ArrowClosed + цвет по статусу)
  ├── Handles скрываются (не удаляются) в Lock mode → связи не пропадают
  ├── Телефон карты 1:1 с менеджером: iframe scale(0.618), пропорции 220×476
  ├── Интерактивный iframe на карте (кнопки, input, options — всегда)
  └── Inline-редактирование текста в менеджере (двойной клик на overlay)
...
### ДОРАБОТКА - ФАЗА 4 является ключевым пунктом, где необходимо проработать быстрое создание веб воронок, на основе html шаблонов, что у нас есть. Необходима Библиотека - Воронок.html элементов для воронки. ###
